...
class ForensicVisualizer:
    ...
    def plot_suspicious_vs_normal(
        self, df, group_col, outcome_vars, suspicious_rows, title=None
    ):
        """
        ...
        if title:
            ...
            fig.suptitle(
                "Comparison of Suspicious vs. Normal Observations", fontsize=16
            )
    ...
    def plot_with_without_comparison(self, comparison_results):
        ...
        sig_change_cols = [
            r["column"] for r in results if r.get("significance_changed", False)
        ]

        # Get group columns if available
        has_groups = (
            "group_column" in comparison_results and "groups" in comparison_results
        )
        ...
        if has_groups:
            ...
            plt.suptitle(
                "Impact of Suspicious Rows on Statistical Analysis", fontsize=16
            )
            ...
            for i, result in enumerate(results):
                ...
                if (
                    "with_suspicious" in result
                    and "group_stats" in result["with_suspicious"]
                ):
                    # Get group stats
                    ...
                    without_means = [
                        without_stats.get(g, {}).get("mean", 0) for g in groups
                    ]
                    without_errors = [
                        without_stats.get(g, {}).get("std", 0) for g in groups
                    ]
                    ...
                    if col_name in sig_change_cols:
                        ...
                        axes[i, 0].set_title(
                            f"Mean Values for {col_name} (Significance Changed!)",
                            color="red",
                        )
                    ...
                    with_values = [
                        result["with_suspicious"]["effect_size"],
                        result["with_suspicious"]["ttest"]["p_value"] * 100,
                    ]
                    without_values = [
                        result["without_suspicious"]["effect_size"],
                    ...
                    axes[i, 1].text(
                        0,
                        result["with_suspicious"]["effect_size"] + 0.1,
                        f"{result['with_suspicious']['effect_size']:.2f}",
                        ha="center",
                    )
                    axes[i, 1].text(
                        1, with_values[1] + 0.1, f"p={p_with:.3f}", ha="center"
                    )
                    axes[i, 1].text(
                        0,
                        result["without_suspicious"]["effect_size"] - 0.3,
                        f"{result['without_suspicious']['effect_size']:.2f}",
                        ha="center",
                    )
                    axes[i, 1].text(
                        1, without_values[1] - 0.3, f"p={p_without:.3f}", ha="center"
                    )
                    ...
                    if "significance_change_description" in result:
                        axes[i, 1].text(
                            0.5,
                            -0.2,
                            result["significance_change_description"],
                            ha="center",
                    ...
                    with_values = [
                        result["with_suspicious"]["mean"],
                        result["with_suspicious"]["std"],
                    ]
                    without_values = [
                        result["without_suspicious"]["mean"],
                        result["without_suspicious"]["std"],
                    ]
                    ...
                    # Add text labels
                    axes[i, 0].text(
                        0, with_values[0] + 0.1, f"{with_values[0]:.2f}", ha="center"
                    )
                    axes[i, 0].text(
                        1, with_values[1] + 0.1, f"{with_values[1]:.2f}", ha="center"
                    )
                    axes[i, 0].text(
                        0,
                        without_values[0] - 0.3,
                        f"{without_values[0]:.2f}",
                        ha="center",
                    )
                    axes[i, 0].text(
                        1,
                        without_values[1] - 0.3,
                        f"{without_values[1]:.2f}",
                        ha="center",
                    )
                    ...
                    axes[i, 1].text(
                        0,
                        result["mean_change_percent"] + 0.1,
                        f"{result['mean_change_percent']:.1f}%",
                        ha="center",
                    )
                    axes[i, 1].text(
                        1,
                        result["std_change_percent"] + 0.1,
                        f"{result['std_change_percent']:.1f}%",
                        ha="center",
                    )
    ...
    def plot_id_sequence(self, df, id_col, group_col, suspicious_rows=None):
        ...
        for i, group in enumerate(groups):
            ...
            plt.plot(
                group_df["row_number"], group_df[id_col], color=colors[i], alpha=0.3
            )
        ...
        if suspicious_rows is not None:
            ...
            plt.scatter(
                suspicious_df["row_number"],
                suspicious_df[id_col],
                color="red",
                s=100,
                marker="x",
                label="Suspicious",
