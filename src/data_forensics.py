...
class DataForensics:
    ...
    def analyze_calc_chain(self, suspect_rows):
        ...
        if not hasattr(self, 'excel_metadata') or not self.excel_metadata.get('calc_chain'):
            return []
        ...
        for row in suspect_rows:
            if row in rows_in_chain:
                ...
                for i, cell in enumerate([c for c in calc_chain if c["cell_ref"] in [x["cell_ref"] for x in cells]]):
    ...
    def analyze_statistical_anomalies(self):
        ...
        for col in numeric_cols:
            ...
            if self.df[col].nunique() < 5 or self.df[col].nunique() > len(self.df) * 0.5:
                continue
    ...
    def analyze_suspicious_observations(self, suspicious_rows, group_col, outcome_vars):
        ...
        for var in outcome_vars:
            try:
                ...
                has_all_groups = all(g in suspicious_df[group_col].unique() for g in groups)
                ...
                effect_size_non_suspicious = group_means_non.max() - group_means_non.min()
                
                # Calculate t-test for suspicious observations
                from scipy import stats
                group_values = [suspicious_df[suspicious_df[group_col] == g][var].values for g in groups]
                ...
                if all(len(values) > 1 for values in group_values):
                    ...
                    try:
                        t_stat, p_value = stats.ttest_ind(group_values[0], group_values[1], equal_var=False)
    ...
    def generate_report(self):
        ...
        for i, finding in enumerate(self.findings):
            ...
            report += f"<h3>Finding {i+1}: {finding['type'].replace('_', ' ').title()}</h3>"
            ...
            if finding['type'] == 'sorting_anomaly':
                ...
                report += f"<p>Detected unusual values in column '{finding['column']}'.</p>"
