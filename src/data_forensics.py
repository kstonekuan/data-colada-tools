...
class DataForensics:
    ...
    def detect_fabrication_patterns(self):
            ...
            try:
                ...
                if last_digit_counts:
                    ...
                    if total_digits > 50:  # Only analyze if we have enough data points
                        expected = total_digits / 10  # Expected uniform count for each digit
                ...
                if len(diffs) > 10:  # Only analyze if enough data points
                    ...
                    if std_diff > 0 and mean_diff > 0 and std_diff / mean_diff < 0.05:
                        ...
                        fabrication_findings.append({
                            ...
                            "details": f"Values show unusually perfect linear progression",
        ...
        for col in analyzed_categorical_cols:
            try:
                ...
                if len(value_counts) >= 5:  # Only meaningful if there are enough distinct values
                    ...
                    if not outliers.empty:
                        ...
                        for value, count in outliers.items():
                            ...
                            if len(row_indices) >= 5:  # Only check if enough occurrences
                                ...
                                if len(gaps) > 5:
                                    ...
                                    gap_std = np.std(gaps)
                                    if gap_std / gap_mean < 0.3:  # Suspiciously consistent spacing
                                        unusual_values.append({
                                            "value": str(value),
                                            "count": int(count),
    ...
    def create_digit_distribution_plot(self, columns):
        ...
        for i, col in enumerate(cols_to_plot):
            try:
                ...
                if expected > 0:
                    axes[i].axhline(y=expected, color='r', linestyle='--', alpha=0.7, 
                                    label=f"Expected ({expected:.1f})")
                    axes[i].legend()
                
            except Exception as e:
                print(f"Error creating plot for {col}: {e}")
                axes[i].text(0.5, 0.5, f"Error creating plot: {str(e)}", 
    ...
    def analyze_terminal_digits(self):
        ...
        for col in numeric_cols:
            ...
            if (self.df[col].nunique() < 10 or 
                np.all(self.df[col] == self.df[col].round())):
                continue
    ...
    def create_terminal_digit_plot(self, anomalies):
        ...
        anomalies_sorted = sorted(anomalies, key=lambda x: x['p_value'])[:min(3, len(anomalies))]
        
        fig, axes = plt.subplots(len(anomalies_sorted), 1, figsize=(10, 5*len(anomalies_sorted)))
        ...
        for i, anomaly in enumerate(anomalies_sorted):
            try:
                ...
                axes[i].axhline(y=expected, color='r', linestyle='--', 
                ...
                axes[i].set_title(f"Terminal Digit Distribution for {col}\nχ²={anomaly['chi_square']:.2f}, p={anomaly['p_value']:.6f}")
    ...
    def analyze_variance_anomalies(self, group_col):
        ...
        for col in analyzed_cols:
            try:
                ...
                min_var = min(group_vars.values()) if min(group_vars.values()) > 0 else float('inf')
    ...
    def create_variance_comparison_plot(self, anomalies, group_col):
        ...
        anomalies_sorted = sorted(anomalies, key=lambda x: x['variance_ratio'], reverse=True)[:min(2, len(anomalies))]
        ...
        for anomaly in anomalies_sorted:
            try:
                ...
                plt.title(f"Variance Comparison for {col} across {group_col}\n" + 
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp:
                    plt.savefig(tmp.name)
                    plt.close()
                    self.plots.append({"column": f"variance_{col}", "plot_path": tmp.name})
    ...
    def create_inlier_distribution_plot(self, anomalies):
        ...
        anomalies_sorted = sorted(anomalies, key=lambda x: x['inlier_percentage'], reverse=True)[:min(2, len(anomalies))]
        ...
        for i, anomaly in enumerate(anomalies_sorted):
            try:
                ...
                axes[i].axvspan(mean - 0.5*std, mean + 0.5*std, alpha=0.2, color='red')
                ...
                for issue in finding["details"]:
                    ...
                    if issue['issue'] == 'excessive_repetition':
                        ...
                        if 'most_common_digit' in issue:
                            report += f", most common digit: {issue['most_common_digit']}"
                    
                    elif issue['issue'] == 'artificial_sequence':
                        report += f"Artificial Sequence - {issue['details']}"
                        if 'sequence_diff_mean' in issue:
                            report += f", mean difference: {issue['sequence_diff_mean']:.3f}"
                    
                    elif issue['issue'] == 'idiosyncratic_clusters':
                        report += f"Idiosyncratic Response Clusters - {issue['details']}"
                        ...
                        if 'unusual_values' in issue and issue['unusual_values']:
                            ...
                            for val_info in issue['unusual_values']:
                                ...
                                if val_info['issue'] == 'sequential_cluster':
                                    ...
                                    report += f" with uniform spacing ({val_info['details']})"
                                report += "</p>"
                            report += "</div>"
